# Servicio A - Sistema de Soporte Tecnico
# Responsabilidad: interfaz de usuario y rutas HTTP
# Este servicio NO tiene time.sleep() ni logica pesada
...
# URL del Servicio B usando el nombre del contenedor como hostname (DNS interno de Docker)
SERVICIO_B_URL = "http://servicio_b:5001"
...
<body>
    ...
    <p>TechNova Solutions — Servicio A (UI)</p>
    ...
    <div class="aviso">Al abrir un ticket, el Servicio B notifica al area tecnica en segundo plano</div>
...
# Ruta 2 - abre un ticket y llama al Servicio B para la notificacion
...
def abrir_ticket():
    ...
    try:
        ...
        # Llamar al Servicio B para que procese la notificacion
        # Se usa el nombre del contenedor como hostname, no la IP
        try:
            respuesta_b = requests.post(
                f"{SERVICIO_B_URL}/notificar",
                json={'ticket_id': ticket_id, 'prioridad': prioridad},
                timeout=15
            )
            notificacion = respuesta_b.json()
        except Exception as e:
            # Si el Servicio B no esta disponible, el ticket igual se guardo
            notificacion = {'estado': 'servicio_b_no_disponible', 'detalle': str(e)}

        return jsonify({
            'mensaje': f'Ticket #{ticket_id} abierto exitosamente.',
            'ticket_id': ticket_id,
            'estado': 'abierto',
            'prioridad': prioridad,
...
# Ruta 3 - consulta tickets abiertos ordenados por prioridad
